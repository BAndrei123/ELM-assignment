...
forward (Cursor l c r) =
    case r of
        [] -> Nothing
        x::xs-> Just (withSelectedElement (List.reverse(c::l)) x xs)
...
back (Cursor l c r) =
    case l of
        [] -> Nothing
        x::xs -> Just (withSelectedElement (List.reverse xs) x (c::r))
...
length : Cursor a -> Int
length (Cursor l c r) =
    (\x y z -> x + y +z) (List.length l) 1 (List.length r)
